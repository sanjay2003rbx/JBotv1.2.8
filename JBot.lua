...
client:on('messageCreate', function(message)
	...
	cmd = cmd or message.content
	if cmd == '=stats' then
		message.channel:sendMessage(string.format(' ``` Name: JBot#7214. Bot: Yes. Desc: Multipurpose bot used to moderator servers or provide fun commands for you and your server. Language: Lua. Framework: Luvit. Lib: Discordia. Lib Creator: Sinister Rectus. Region: World. Version: 1.2.8. Updated: Yes. Updates: No. ```, %s', message.author:getMentionString()))
		end
	end
end)
...
client:on('messageCreate', function(message)
	...
	cmd = cmd or message.content
	if cmd == '=cmds' then
		message.channel:sendMessage(string.format(' ``` Comamnds: @Creator: =test, =servers, =stats, =disconnect @Admin: =kick <@user>, =ban <@user>, =admins, =creator @Free: =cmds, =joke, =invite, prefix, =about, =home, =new ```, %s', message.author:getMentionString()))
...
client:on('messageCreate', function(message)
	...
	cmd = cmd or message.content
	if cmd == '=new' then
		message.channel:sendMessage(string.format(' ``` Current Version 1.2.8: Added "=disconnect" to disconnect the bot for maintenance. Fix minor bugs. ```, %s', message.author:getMentionString()))
